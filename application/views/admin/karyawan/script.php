<script>
    ...
    function detail(id)
    {
        ...
        $.ajax({
            ...
            {
                var fileUrl = base_url +'assets/img/karyawan/'+ data.gambar;
                check_file_exists(fileUrl, function(exists)
                {
                    if(exists)
                    {
                        $("#img").html('<img src="'+ base_url +'assets/img/karyawan/'+ data.gambar +'" class="img img-fluid img-thumbnail" width="120px">');
                    }
                });
