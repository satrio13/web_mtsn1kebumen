...
class Prestasi_siswa_model extends CI_Model {
    ...
    private $table = 'tb_prestasi_siswa p'; //nama tabel dari database
    private $column_order = array(null,'t.tahun','p.nama','p.jenis','p.prestasi','p.nama_siswa','p.kelas','p.tingkat','p.tingkat','p.tingkat','p.tingkat','p.tingkat','p.keterangan','p.gambar','p.id');
    private $column_search = array('t.tahun','p.nama','p.jenis','p.prestasi','p.nama_siswa','p.kelas','p.tingkat','p.keterangan','p.gambar');
	...
	{
        $config['upload_path'] = 'assets/img/prestasi/siswa/';
        ...
        if(!empty($_FILES['gambar']['name']))
        {
            if($this->upload->do_upload('gambar'))
            {
                ...
                $this->db->insert('tb_prestasi_siswa',$data_insert);
                if($this->db->affected_rows() > 0)
                {
                    $this->session->set_flashdata('msg-prestasi', 'PRESTASI BERHASIL DITAMBAHKAN');
                    redirect('backend/prestasi-siswa');
                }else
                {
    ...
    {
        return $this->db->select('id')->from('tb_prestasi_siswa')->where('id',$id)->get()->row();
    }

    function edit_prestasi_siswa($id)
	{
        $get = $this->db->select('id,gambar')->from('tb_prestasi_siswa')->where('id',$id)->get()->row();
        $target = "assets/img/prestasi/$get->gambar";
        ...
        if(!empty($_FILES['gambar']['name']))
        {
            if($this->upload->do_upload('gambar'))
            {
                ...
                $this->db->update('tb_prestasi_siswa',$data_update, ['id'=>$id]);
                if($this->db->affected_rows() > 0)
                {
	...
	{
        $get = $this->db->select('id,gambar')->from('tb_prestasi_siswa')->where('id',$id)->get()->row();
        $target = "assets/img/prestasi/siswa/$get->gambar";
		unlink($target);
		$this->db->delete('tb_prestasi_siswa', array('id'=>$id));
        if($this->db->affected_rows() > 0)
        {
