...
class Prestasi_guru_model extends CI_Model {
    ...
    private $table = 'tb_prestasi_guru p'; //nama tabel dari database
    private $column_order = array(null,'t.tahun','p.nama','p.jenis','p.prestasi','p.nama_guru','p.tingkat','p.tingkat','p.tingkat','p.tingkat','p.tingkat','p.keterangan','p.gambar','p.id');
    private $column_search = array('t.tahun','p.nama','p.jenis','p.prestasi','p.nama_guru','p.tingkat','p.keterangan','p.gambar');
	...
	{
        $config['upload_path'] = 'assets/img/prestasi/guru/';
    ...
    {
        return $this->db->select('id')->from('tb_prestasi_guru')->where('id',$id)->get()->row();
    }

    function edit_prestasi_guru($id)
	{
        $get = $this->db->select('id,gambar')->from('tb_prestasi_guru')->where('id',$id)->get()->row();
        $target = "assets/img/prestasi/guru/$get->gambar";
        ...
        if(!empty($_FILES['gambar']['name']))
        {
            if($this->upload->do_upload('gambar'))
            {
                ...
                $this->db->update('tb_prestasi_guru',$data_update, ['id'=>$id]);
                if($this->db->affected_rows() > 0)
                {
    ...
    function hapus_prestasi_guru($id)
	{
        ...
        $target = "assets/img/prestasi/guru/$get->gambar";
		unlink($target);
		$this->db->delete('tb_prestasi_guru', array('id'=>$id));
        if($this->db->affected_rows() > 0)
        {
